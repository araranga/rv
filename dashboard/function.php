...
function getLatestId($table,$id){


$q = mysql_query_md("SELECT $id FROM $table  ORDER by $id DESC LIMIT 1");
...
if($_GET['pages']=='reviews' || $_GET['pages']=='articles' || $_GET['pages']=='paints' || $_GET['pages']=='report' || $_GET['pages']=='withdraw' || $_GET['pages']=='exchange' || $_GET['pages']=='payments' || $_GET['pages']=='bonuses')
...
if(!empty($poke['is_market'])){
	echo "Pokemon is not able to battle since this is on sale on Market.";
	exit();	
}
