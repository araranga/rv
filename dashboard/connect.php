...
function getconnection(){
  ...
  return new mysqli("localhost","root","root","rvcustom",3307);
            ...
            function loadform($field,$sdata = array(),$is_editable_field = 1)
            {
               ...
               foreach($field as $inputs)
               {
                  ...
                  $return .= "<select class='form-control' name=\"{$inputs['value']}\" id=\"{$inputs['value']}\" required {$inputs['attr']}>";
					  ...
					  $editor = $sdata[$inputs['value']];
                      $sdata[$inputs['value']] = htmlentities($sdata[$inputs['value']]);

					  if($_GET['task']!='delete') {
                      $return .= "<textarea style='height:600px'  class='editor form-control' {$inputs['attr']} name=\"{$inputs['value']}\" id=\"{$inputs['value']}\" name=''>{$sdata[$inputs['value']]}</textarea>";
                      }else{
						$return .= $editor;
					  }
						...
						$path_file = $sdata[$inputs['filedata']];
						$path_type = $inputs['filetype'];
						$path_folder = $inputs['path'];
						
						
						
						if($_GET['task']!='delete') {
						$return .= "<input class='form-control' $stepany {$inputs['attr']} type=\"{$inputs['type']}\" name=\"{$inputs['value']}\" id=\"{$inputs['value']}\" size=\"40\" maxlength=\"255\" value=\"{$sdata[$inputs['value']]}\" />";
						$return .= "<span class=\"validation-status\"></span>";
						}
						
						if($path_type=='image' && $_GET['task']!='add'){
							
							$return .= "<div style='margin: 0 auto;padding-top: 13px;'>";
							
							$finalimg = $path_folder."/".$path_file;
							
							$return .= "<img src='{$finalimg}' width='250'>";
